...
OPUS_LIBRARY_PATH = {
    ...
    "Windows": "libopus-0.dll"  # Windows용 라이브러리 경로
}
...
async def on_message(message):
    ...
    if message.channel.name == 'tts':
        ...
        async def handle_tts(text, lang, guild_id):
            tts_file = f"tts_audio_{guild_id}.mp3"
            try:
        ...
        await handle_tts(message.content, language, message.guild.id)
